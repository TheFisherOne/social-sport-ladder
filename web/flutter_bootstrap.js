...
if (loading) {
  loading.textContent = "Fetching Social Sport Ladder files from web server...";  // Stage 1: entrypoint loading
}
...
_flutter.loader.load({
  ...
  onEntrypointLoaded: async function(engineInitializer) {
      if (loading) {
        loading.textContent = "Initializing Flutter engine...";  // Stage 2: renderer/assets download
      }

      const appRunner = await engineInitializer.initializeEngine();

      if (loading) {
        loading.textContent = "Starting application... Almost there!";  // Stage 3: running app
      }
